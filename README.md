...
托盘菜单提供显示/隐藏悬浮球、立即刷新、刷新间隔（1、2、5、10、30 分钟）、通知、始终置顶、开机启动、跟随 Codex、面板常驻和退出。
...
- **额度重置提醒**：某个额度窗口被服务端换到下一个周期时弹一次（5 小时窗口与周/月窗口各自判断）。判定依据是新旧快照里该窗口的重置时间恰好前移了一个窗口长度——只看“已过重置时间”或“百分比回落”都会把切换账号误判成重置。
- **余额使用提醒**：本周期首次动用余额（credits 溢出实扣 ≥ 0.25 credits）时弹一次，正文按原始口径显示 credits，美元是括号里的换算值。要求先观测到“本期还没溢出”，再观测到溢出，因此程序启动时本期已经在扣的情况不会补弹。
