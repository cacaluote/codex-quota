...
托盘菜单提供显示/隐藏悬浮球、立即刷新、复制面板截图、刷新间隔（1、2、5、10、30 分钟）、通知、始终置顶、开机启动、跟随 Codex、面板常驻和退出。

“复制面板截图”把当前数据对应的完整展开面板画进一张 288 DIP 宽、按当前显示器 DPI 缩放的位图并复制到剪贴板，不影响悬浮窗当前的隐藏、收起或展开状态：悬浮球收起或隐藏时截出来的同样是完整面板。截图是不透明直角图，透明圆角与半透明底都会铺成面板色，粘贴到聊天、文档或只认 `CF_DIB` 的软件里都不会出现白边或黑角；圆角外观只在桌面悬浮窗上保留。剪贴板同时带 `CF_DIB`/`CF_BITMAP` 和 PNG 虚拟文件：前者给只认旧位图格式的程序，后者可在资源管理器文件夹里 Ctrl+V 得到 `CodexQuota.png`。剪贴板被其他程序占用时本次复制失败并在托盘提示重试。
...
“复制面板截图”的成功与失败提示走的是同一套托盘气泡，但不受上面两个开关影响；同样受专注助手约束，因此它也可能被系统吞掉，操作结果以日志为准。
